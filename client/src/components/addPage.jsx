...
export function AddPage(props){
    ...
    const [author, setAuthor] = useState(props.user.id);
    const [authors, setAuthors] = useState([]);

    useEffect(() => {
      const fetchAuthors = async () => {
        try{
          if(props.user.role !== 1) return;
    ...
    const handleSubmit = async () => {
        try{
          ...
          const fullPage = {title: title, publicationdate: pubdate, idauthor: author, blocks: fullBlocks}
          await API.addPage(fullPage);
          navigate('/');
        } catch(err){
    ...
    const renderFields = () => {
            ...
            <Row key={index}>&nbsp;
                ...
                <Col sm={6}>
                  {field.type === 'I' ? (
                    ...
                    <Form.Control className="form-select" as="select" name="value" value={field.value} placeholder="Select image" style={{border: '1px solid'}} onChange={(event) => handleChange(index, event)}>
        ...
        <Col sm={2}>
          {index > 0 && (
            <Row>
            <Col sm={3}><Button style={{ marginTop: '-66px'}} variant="info" hidden={index===1} onClick={() => handleMoveUp(index)}>
            <i className="bi bi-arrow-up-circle"></i>
            </Button></Col>
            <Col sm={3}><Button style={{ marginTop: '-66px'}} variant="info" hidden={index===inputFields.length-1} onClick={() => handleMoveDown(index)}>
            <i className="bi bi-arrow-down-circle"></i>
            </Button></Col>
            <Col sm={3}><Button style={{ marginTop: '-66px'}} variant="danger" onClick={() => handleRemoveFields(index)}>
    ...
    return(
      ...
      <Form onSubmit={(ev) => {ev.preventDefault()} }>
        ...
        <Form.Control type="text" name="title" style={{border: '1px solid'}} onChange={(ev) => setTitle(ev.target.value)}/>
        </div>
        {props.user.role ? <>
        ...
        <div className="col-sm-2">
        <Form.Control type="date" name="pubdate" style={{border: '1px solid'}} onChange={(ev) => setPubdate(ev.target.value)}/>
        </div>
      </Form.Group>
